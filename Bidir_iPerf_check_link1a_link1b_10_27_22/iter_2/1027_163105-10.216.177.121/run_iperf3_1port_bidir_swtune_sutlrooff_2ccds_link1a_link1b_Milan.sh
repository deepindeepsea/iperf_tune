...
do
    ...
    ethtool -L link1a combined 8
    ip link set link1a mtu 1500
    systemctl start irqbalance
    sleep 5
    systemctl stop irqbalance
    for f in /sys/class/net/link1a/queues/rx-*/rps_flow_cnt; do echo 8192 >"$f"; done
    ...
    for f in /sys/class/net/link1b/queues/rx-*/rps_flow_cnt; do echo 8192 >"$f"; done
    ...
    ssh -p 22 ${REMOTE_HOST} "ethtool -L link1a combined 8"
    ssh -p 22 ${REMOTE_HOST} "ip link set link1a mtu 1500"
    ssh -p 22 ${REMOTE_HOST} "for f in /sys/class/net/link1a/queues/rx-*/rps_flow_cnt; do echo 8192 > \$f; done"
    ...
    ssh -p 22 ${REMOTE_HOST} "ethtool -L link1b combined 8"
    ssh -p 22 ${REMOTE_HOST} "ip link set link1b mtu 1500"
    ssh -p 22 ${REMOTE_HOST} "for f in /sys/class/net/link1b/queues/rx-*/rps_flow_cnt; do echo 8192 > \$f; done"
    ssh -p 22 ${REMOTE_HOST} "/usr/sbin/set_irq_affinity_cpulist.sh \
